...
let yourGrassImage = new Image();
yourGrassImage.src = 'tileableGrass.png';

let yourGrassRockImage = new Image();
yourGrassRockImage.src = 'tileablerockGrass.png';


let leftRoadImage = new Image();
leftRoadImage.src = 'roadLeft.png';


let rightRoadImage = new Image();
rightRoadImage.src = 'roadRight.png';


let topRoadImage = new Image();
topRoadImage.src = 'roadTop.png';

let bottomRoadImage = new Image();
bottomRoadImage.src = 'roadBottom.png';


let dirtPathVertImage = new Image();
dirtPathVertImage.src = 'dirtPathVert.png';
...
function drawFloor(px, py, ratio) {
    ...
    for (let i = 0; i < H; i++) {
        ...
        let top = (ratio * i * global.gameHeight) / H - py + global.screenHeight / 2,
        ...
        for (let j = 0; j < W; j++) {
            ...
            let tile = row[j];
            ctx.globalAlpha = 1;
            if (tile.startsWith('green')) {
                ctx.drawImage(yourGrassImage, left, top, right - left, bottom - top);
            } 
            else if (tile.startsWith('grey')){
                ctx.drawImage(yourGrassRockImage, left, top, right - left, bottom - top);
            }
            else if (tile.startsWith('red')){
                ctx.drawImage(leftRoadImage, left, top, right - left, bottom - top);
            }
            else if (tile.startsWith('blue')){
                ctx.drawImage(rightRoadImage, left, top, right - left, bottom - top);
            }
            else if (tile.startsWith('yellow')){
                ctx.drawImage(topRoadImage, left, top, right - left, bottom - top);
            }
            else if (tile.startsWith('darkGreen')){
                ctx.drawImage(bottomRoadImage, left, top, right - left, bottom - top);
            }
            else if (tile.startsWith('dirtPathSide')){
                ctx.drawImage(dirtPathSideImage, left, top, right - left, bottom - top);
            }
...
function drawMinimapAndDebug(spacing, alcoveSize) {
    ...
    // Text
    if (global.showDebug) {
        drawText("ArrasPolice.CC", x + len, y - 50 - 5 * 14 - 2, 15, "#027822", "right");
        ...
        drawText("ArrasPolice.CC", x + len, y - 50 - 2 * 14 - 2, 15, "#027822", "right");
        drawText((100 * gui.fps).toFixed(2) + "% : " + global.metrics.rendertime + " FPS", x + len, y - 50 - 1 * 14, 10, global.metrics.rendertime > 10 ? color.guiwhite : color.orange, "right");
